...
class WaiverRequest(Document):
    ...
    def validate_sales_order(self):
        ...
        if sales_order.docstatus != 0:
            frappe.throw(f"Sales Order <b>{self.sales_order}</b> is not a draft, a waiver cannot be requested on it")
    ...
    def before_submit(self):
        ...
        if self.decision == "Approved" and not self.additional_discount_percentage and not self.discount_amount:
...
@frappe.whitelist()
def create_waiver_request(sales_order, apply_discount_on, waiver_reason, additional_discount_percentage=0, discount_amount=0):
