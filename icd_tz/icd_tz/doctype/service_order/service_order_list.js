frappe.listview_settings["Service Order"] = {
  ...
  onload: (listview) => {
    ...
    listview.page
      .add_inner_button(__("Create Bulk Service Order"), () => {
    ...
    action: function (doc) {
      frappe.call({
        ...
        callback: function (response) {
          if (response.message) {
            frappe.show_alert(
              __("Service Order: {0} submitted successfully.", [doc.name]),
              5
            );
          } else {
            frappe.show_alert(
              __("Failed to submit Service Order {0}.", [doc.name]),
              5
            );
          }
        },
  ...
  let d = new frappe.ui.Dialog({
    ...
    primary_action(values) {
      if (values) {
        ...
        frappe.call({
          method:
            "icd_tz.icd_tz.doctype.service_order.service_order.create_bulk_service_orders",
          args: {
            data: values,
          },
          freeze: true,
          ...
          callback: (r) => {
            if (r.message) {
              ...
              frappe.show_alert(
                {
                  message: __("{0} Service Orders Created successfully", [
                    r.message,
                  ]),
                  indicator: "green",
                },
